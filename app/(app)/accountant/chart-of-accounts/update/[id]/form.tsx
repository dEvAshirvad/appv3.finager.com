...
function UpdateAccountForm({
	...
	const { mutate: updateAccount, isPending } = useUpdateCOA();
	...
	// Filter accounts - only exclude current account (API already filters by search and type)
	const displayAccounts = React.useMemo(() => {
		if (!parentAccountsData?.data?.docs) return [];
		return parentAccountsData.data.docs.filter(
			(account) => account._id !== accountId
		);
	}, [parentAccountsData?.data?.docs, accountId]);
	...
	if (!accountData?.data || !accountData.data) {
		return (
			<div className="flex items-center justify-center h-64">
