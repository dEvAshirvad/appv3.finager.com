...
function CreateItemForm({ className, ...props }: React.ComponentProps<"form">) {
	...
	// COA Account searches - always fetch, show all by default
	const { data: salesAccountsDataRaw } = useCOAList({
		organizationId: sessionData?.session?.activeOrganizationId,
		page: 1,
		limit: debouncedSalesAccountSearch.length > 0 ? 50 : 100,
		search:
			debouncedSalesAccountSearch.length > 0
				? debouncedSalesAccountSearch
				: undefined,
	});
	...
	const { data: purchaseAccountsDataRaw } = useCOAList({
		organizationId: sessionData?.session?.activeOrganizationId,
		page: 1,
		limit: debouncedPurchaseAccountSearch.length > 0 ? 50 : 100,
		search:
			debouncedPurchaseAccountSearch.length > 0
				? debouncedPurchaseAccountSearch
				: undefined,
	});
	...
	const { data: inventoryAccountsDataRaw } = useCOAList({
		organizationId: sessionData?.session?.activeOrganizationId,
		page: 1,
		limit: debouncedInventoryAccountSearch.length > 0 ? 50 : 100,
		search:
			debouncedInventoryAccountSearch.length > 0
				? debouncedInventoryAccountSearch
				: undefined,
	});
